...
class Renderer:
    ...
    def __call__(self, state: PosterState) -> PosterState:
        ...
        output_dir = Path(state["output_dir"]).parent
    ...
    def _select_poster_layout(self, state: PosterState, section_info: Dict, poster_layouts_file: Path):
        with open(poster_layouts_file, "r") as f:
            ...
            poster_layout = poster_layouts_data[poster_layout_id+1]
    ...
    def _select_poster_layout_id(self, state: PosterState, section_info: Dict, poster_layouts_data: Dict):
        ...
        for section in sections:
            ...
            if section_title.lower() == "title_author":
                ...
                text_area = self.__caculate_title_text_area(state, title_font_size)
                visual_assets_area = self.__caculate_visual_area(visual_assets, tables, figures)#这里很可能没有素材，我的想法是后面补上默认图标
                ...
                visual_assets = section["visual_assets"]
                visual_assets_area = self.__caculate_visual_area(visual_assets, tables, figures)
                text_area = self.__caculate_text_area(section_content, text_font_size)
                section_areas[section_title] = visual_assets_area + text_area
                total_area += visual_assets_area + text_area
        # 对面积排序
    ...
    def __caculate_title_text_area(self, state:PosterState, title_font_size=100) ->float:
        title_author = state["narrative_content"]["meta"]
        title_author_len = len(title_author["poster_title"]) + len(title_author["author"])
        ratio = title_font_size/72
        text_area = (ratio*ratio)*(52*39)/(16*12)*len(title_author_len) 
        
        return text_area

    def __caculate_visual_area(self, visual_assets: Dict, tables: Dict, figures: Dict) -> float:
    ...
    def __caculate_text_area(self, text_content: Dict, text_font_size=36) -> float:
        """ 计算文本占面积的大小 """
        for line in text_content:
            text_len += len(line)
        ratio = text_font_size/72
        text_area = (ratio*ratio)*(52*39)/(16*12)*text_len
    ...
    def _set_layout(self, section_info: Dict, poster_layout: Dict, sorted_area) -> Dict:
        sections_layout = []
        sections = section_info["spatial_content_plan"]["sections"]
        for id, info in enumerate(sorted_area.items()):
            section_title = info[0]
            section_location = poster_layout["layout"][i]
            for section in sections:
    ...
    def render_poster(self, state: PosterState, sections_layout: Dict):
        ...
        for section_layout in sections_layout:
            ...
            ratio = 3.25
            container = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(x*ratio), Inches(y*ratio), Inches(width*ratio), Inches(height*ratio))
            container.line.color.rgb = RGBColor(0, 0, 255)
            textbox = container.textbox
            textbox.text = section_layout["section_title"] + "\r\n"
            tf = textbox.text_frame
            tf.paragraphs[0].font.size = Pt(36)
            tf.paragraphs[0].font.bold = True
            tf.paragraphs[0].alignment = PP_ALIGN.CENTER
            tf.paragraphs[0].vertical_anchor = MSO_VERTICAL_ANCHOR.TOP
            for line in section_layout["section_content"]:
                ...
                tf.paragraphs[-1].vertical_anchor = MSO_VERTICAL_ANCHOR.TOP
                tf.paragraphs[-1].text = line + "\r\n"
            
        prs.save(state["output_dir"] / "poster.pptx")
        return state
